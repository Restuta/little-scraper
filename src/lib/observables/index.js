...
const createScraper = ({
  ...
  return ({ fromUrls: urlsWithContext, fromUrlsIterator: urlsIterator }) => {
    ...
    const fromUrlsIterator = async (urlsIterator, scrapeWhile) => {
      ...
      const scrapingPromise = urlsIteratorSubject.pipe(map(url => (R.type(url) === 'String' ? { url } : url)), mergeMap(({ url }) =>
        Rx.of(url).pipe(flatMap(url => httpGet(url)), rxjsDelay(getDelay()), retryWhen(httpError({
      ...
      (urlWithContext, response) => ({ response, urlWithContext }), concurrency), tap(({ response, urlWithContext }) => {
        ...
        }), map(scrapingFunc), reduce((results, currentResults) => {
